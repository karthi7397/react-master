...
export default function UserSelection({ selectUser }) {
  ...
  useEffect(() => {
    ...
    const storedResults = localStorage.getItem('react-master_quizResults');
    if (storedResults) {
  ...
  const addUser = (name) => {
    ...
    localStorage.setItem('react-master_quizUserId', newUserId);
    ...
    localStorage.setItem('react-master_quizResults', JSON.stringify(updatedUsers));
