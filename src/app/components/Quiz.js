...
export default function Quiz({ quizType, user, resetQuiz }) {
  ...
  const finishQuiz = () => {
    ...
    const currentUserId = localStorage.getItem('react-master_quizUserId');

    if (currentUserId) {
      // Get existing quiz results from local storage
      const storedResults = JSON.parse(localStorage.getItem('react-master_quizResults') || '[]');
      ...
      localStorage.setItem('react-master_quizResults', JSON.stringify(updatedResults));
    }
  };
